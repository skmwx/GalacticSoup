  ...
  it('runs ordered migrations and marks what it applied [TECH-11.4]', () => {
    ...
    const migrations: readonly SaveMigration[] = [
      ...
      },
      {
        from: 7,
        to: 8,
        describe: 'seventh test step',
        migrate: (save) => ({ ...save, migratedSeventh: true }),
      },
      {
        from: 8,
        to: 9,
        describe: 'eighth test step',
        migrate: (save) => ({ ...save, migratedEighth: true }),
      },
      {
        from: 9,
        to: 10,
        describe: 'ninth test step',
        migrate: (save) => ({ ...save, migratedNinth: true }),
      },
    ];
    ...
    expect(result.ok).toBe(true);
    expect(result.ok && result.migrated).toEqual([1, 2, 3, 4, 5, 6, 7, 8, 9]);
  });
