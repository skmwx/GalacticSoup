...
import { nextBoundary, takeBoundaryDue } from './scheduler';
...
/** A continuously integrated system, run before completions at the same time. */
export type ContinuousSystem = (
...
export function advanceTime(
  ...
  continuousSystems: readonly ContinuousSystem[] = [],
): AdvanceOutcome {
  ...
  while (time.accumulatorMs >= rules.simulationQuantumMs) {
    ...
    boundaries += resolveThrough(context, target, resolvers, continuousSystems);
...
function resolveThrough(
  ...
  while (context.draft.time.simulationTimeMs < targetMs || nextBoundary(context.draft.scheduler)?.dueAtMs === targetMs) {
    ...
    if (entry === null) {
      if (stepEnd >= targetMs) return resolved;
      continue;
    }
    if (resolved >= MAX_BOUNDARIES_PER_QUANTUM) return resolved;

    context.draft.time.simulationTimeMs = entry.dueAtMs;
